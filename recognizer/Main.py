...
class DetectChars:
    ...
    ###################################################################################################
    def loadKNNDataAndTrainKNN(textt,texty):
        allContoursWithData = []                # declare empty lists,
        validContoursWithData = []              # we will fill these shortly

        try:
            npaClassifications = np.loadtxt(textt, np.float32)                  # read in training classifications
        ...
        try:
            npaFlattenedImages = np.loadtxt(texty, np.float32)                 # read in training images
        except:                                                                                 # if file could not be opened
    ...
    def detectCharsInPlates(listOfPossiblePlates):
        ...
        for possiblePlate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
            ...
            listOfPossibleCharsInPlate = DetectChars.findPossibleCharsInPlate(possiblePlate.imgGrayscale, possiblePlate.imgThresh)
            ...
            possiblePlate.strChars = DetectChars.recognizeCharsInPlate(possiblePlate.imgThresh, longestListOfMatchingCharsInPlate)
 ...
 ###################################################################################################
    def findListOfListsOfMatchingChars(listOfPossibleChars):
        ...
        for possibleChar in listOfPossibleChars:                        # for each possible char in the one big list of chars
            listOfMatchingChars = DetectChars.findListOfMatchingChars(possibleChar, listOfPossibleChars)        # find all chars in the big list that match the current char
            ...
            recursiveListOfListsOfMatchingChars = DetectChars.findListOfListsOfMatchingChars(listOfPossibleCharsWithCurrentMatchesRemoved)      # recursive call
...
def findPossibleCharsInScene(imgThresh):
    ...
    contours, Hierarchy = cv2.findContours(imgThreshCopy, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)   # find all contours
...
###################################################################################################
class Main:
    def main(file,textt,texty):

        blnKNNTrainingSuccessful = DetectChars.loadKNNDataAndTrainKNN(textt,texty)         # attempt KNN training
        ...
        imgOriginalScene  = cv2.imdecode(np.fromstring(file.read(),np.uint8), cv2.IMREAD_UNCHANGED)               # open image
        ...
        if len(listOfPossiblePlates) == 0:                          # if no plates were found
            ...
            Main.drawRedRectangleAroundPlate(imgOriginalScene, licPlate)             # draw red rectangle around plate

            print("\nlicense plate read from image = " + licPlate.strChars + "\n")  # write license plate text to std out
            print("----------------------------------------")
            sagar = "\nlicense plate read from image = " + licPlate.strChars + "\n"

            Main.writeLicensePlateCharsOnImage(imgOriginalScene, licPlate)           # write license plate text on the image
